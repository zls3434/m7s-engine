...
func findOrCreateStream(streamPath string, waitTimeout time.Duration) (s *Stream, created bool) {
	...
	pl := len(p)
	if pl < 2 {
		log.Warn(aurora.Red("Stream Path Format Error:"), streamPath)
		return nil, false
	}
...
func (r *Stream) action(action StreamAction) (ok bool) {
	...
	if next, ok = event.Next(); ok {
		...
		var stateEvent any
		r.Info(aurora.Sprintf("%s%s%s", event.From.String(), aurora.Yellow("->"), next.String()), zap.String("action", action.String()))
		switch next {
